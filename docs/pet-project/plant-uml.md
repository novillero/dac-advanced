...
Диаграмма последовательности (sequence diagram) показывает взаимодействие между объектами в системе: как, кем и в каком порядке выполняются разные операции. С помощью диаграммы последовательности можно описать любой процесс в системе, например, пользовательский сценарий.
